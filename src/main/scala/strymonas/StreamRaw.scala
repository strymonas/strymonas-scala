...
object StreamRaw {
   ...
   def foldRaw[A: Type](consumer: A => Cde[Unit], st: StreamShape[A])(using Quotes): Cde[Unit] = {

      def consume[A: Type](g: Goon, consumer: A => Cde[Unit], st: Producer[A])(using Quotes): Cde[Unit] = {
         st match {
            case For(pullArray) =>
      ...
      def loop[A : Type](consumer: A => Cde[Unit], st: StreamShape[A])(using ctx: Quotes): Cde[Unit] = {
         st match {
   ...
   def normalizeFlat[A: Type](fl: Flat[A])(using Quotes): Flat[A] = {
      fl match {
   ...
   def mapRaw_CPS[A: Type, B: Type](tr: A => Emit[B], s: StreamShape[A])(using Quotes): StreamShape[B] = {
      s match {
   ...
   def mapRaw_Direct[A: Type, B: Type](f: A => B, s: StreamShape[A])(using Quotes): StreamShape[B] = {
   ...
   def linearize[A: Type](st: StreamShape[A])(using ctx: Quotes): StreamShape[A] = {
      ...
      def nested[A: Type, B: Type](gouter: Goon, next : Cde[B] => StreamShape[A], stt: Flat[Cde[B]])(using ctx: Quotes): StreamShape[A] = {
         stt match { 
      ...
      def split_init[A: Type, W](init: Cde[Unit], st: StreamShape[A], k: (Cde[Unit] => (Goon, Emit[A]) => StreamShape[W]))(using ctx: Quotes): StreamShape[W] = 
         st match{
            case Initializer(ILet(i, t), sk) => 
   ...
   def zipRaw[A: Type, B: Type](st1: StreamShape[A], st2: StreamShape[B])(using Quotes): StreamShape[(A, B)] = {
