...
trait Stream_Raw {
  ...
  def mkInitUArr[Z, A](size: Int, init: Cde[Z], sk: Cde[Array[Z]] => Stream[A])(using
      t: Type[Z],
      ct: ClassTag[Z]
  ): Stream[A]
...
class Raw(val code: CdeSpec[Code.Cde]) extends Stream_Raw {
  ...
  def predsConj[A](preds: List[A => Cde[Boolean]])(using Quotes): (A => Cde[Boolean]) = { x =>
    preds.map(pred => pred(x)).reduceRight((a, b) => a && b)
  }
  ...
  def mkInitArr[Z, A](init: Array[Cde[Z]], sk: Cde[Array[Z]] => Stream[A])(using
      t: Type[Z],
      ct: ClassTag[Z]
  ): Stream[A] = {
      ...
      case (m, g, For(array)) =>
        mkInitVar(
          ...
          i =>
            Flattened(
              m,
              goon_conj(g, GExp(dref(i) <= array.upb())),
              Unfold((k: A => Cde[Unit]) => seq(array.index(dref(i))(k), incr(i)))
            )
        )
    }
  }
    ...
    def consume[A](g: Goon, consumer: A => Cde[Unit], st: Producer[A])(using Quotes): Cde[Unit] = {
      st match {
        case For(pullArray) =>
          ...
          for_(pullArray.upb(), bp, (i: Cde[Int]) => pullArray.index(i)(consumer))
        case Unfold(step)   =>
  ...
  def linearize[A](st: Stream[A])(using ctx: Quotes): Stream[A] = {
                                  ...
                                  while_(dref(again))(
                                    seq(
                                      if1(
                                        ...
                                        if_(
                                          cde_of_goon(g),
                                          step(x => seq(xres := x, seq(i_, in_inner := cde_of_goon(g_)))),
                                          seq(goon := bool(false), again := bool(false))
                                        )
                                      ),
                                      if1(
                                        dref(in_inner),
                                        seq(step_(x => seq(k(x), again := bool(false))), in_inner := cde_of_goon(g_))
                                      )
                                    )
                                  )
                                )
    ...
    def split_init[A, W](init: Cde[Unit], st: Stream[A], k: (Cde[Unit] => (Goon, Emit[A]) => Stream[W]))(using
        ctx: Quotes
    ): Stream[W] =
      st match {
        case Initializer(ILet(i, t), sk)                            =>
        ...
        case Initializer(IArr(a, t, ct), sk)                        =>
          ...
          def applyLet[B: Type: ClassTag](a: Array[Cde[B]], sk: (Cde[Array[B]] => Stream[A])): Stream[W] = {
            def loop(i: Int, zres: Cde[Array[B]], acc: Cde[Unit]): Cde[Unit] = {
              if i >= len then acc
              else loop(i + 1, zres, seq(acc, array_set(zres)(int(i))(a(i))))
            }
            mkInitUArr[B, W](
              len,
              uninit[B],
    ...
    def mmain[A](unn: Boolean, st: Stream[A]): Stream[A] = {
      st match {
        ...
        case Nested(g, sf @ (_, _, For(_)), t, next)      =>
          def applyNested[B: Type](g: Goon, sf: Flat[Cde[B]], next: Cde[B] => Stream[A]): Stream[A] = {
            mmain[A](unn, guard(g, flatMapRaw(next, for_unfold(sf))))
          }
          applyNested(g, sf, next)(t)
        case Nested(g, sf @ (Filtered(_), _, _), t, next) =>
          def applyNested[B: Type](g: Goon, sf: Flat[Cde[B]], t: Type[B], next: Cde[B] => Stream[A]): Stream[A] = {
